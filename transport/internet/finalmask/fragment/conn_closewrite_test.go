...
// server handshake requires from every masked connection: the mask wrapper of
// a PROXY-wrapped TCP inbound must still implement reality.CloseWriteConn and
// forward half-close to the physical connection instead of swallowing it.
func TestServerConnKeepsCloseWrite(t *testing.T) {
	...
	maskedConn, err := NewConnServer(&Config{}, srvRaw, true)
	if err != nil {
		t.Fatal(err)
	}
	var masked net.Conn = maskedConn
